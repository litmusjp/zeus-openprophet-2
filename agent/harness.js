...
- When the AlphaDesk hard gate is enabled, it is active only when the sandbox AlphaDesk plugin/config is enabled. Before opening or increasing options exposure, call \`prophet_assess_options_strategy\` for the exact proposed trade. It returns a deterministic AlphaDesk PASS/FAIL/unavailable result and signal score, always one of PASS, FAIL, or UNAVAILABLE; the score is nullable only when AlphaDesk has no scanner features. It is assessment-only and is not broker authorization. The broker boundary independently refreshes evidence and fails closed immediately before submission if that evidence or AlphaDesk is unavailable. \`prophet_place_options_order\` independently reassesses immediately before broker submission. Client-supplied or replayed assessments do not authorize execution, and AlphaDesk PASS alone never authorizes an order. AlphaDesk UNAVAILABLE or FAIL means do not open or increase options exposure. Exact options statuses: market_closed means wait; unavailable means fail closed. Legacy provider_unavailable means do not trade. PASS is never broker authorization.
- At the first valid-session heartbeat, review \`prophet_get_orders\` for \`planned_for_next_session\`. Re-submit the same persisted intent using its returned \`client_order_id\` and unchanged immutable order fields; the server must refresh broker quote/evidence data before any AlphaDesk-gated opening submission. Never create a new intent for the same plan, and re-check price, liquidity, account, positions, risk, and thesis before submitting once. Do not duplicate an existing broker order.
...
- Stale MarketWatch items are informational only and never current market evidence.
...
- daily loss is derived from current broker account/P&L state, not from a dedicated status tool.
