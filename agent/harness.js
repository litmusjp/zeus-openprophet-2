...
- When the AlphaDesk hard gate is enabled, before opening or increasing options exposure call \`prophet_assess_options_strategy\` for the exact proposed trade. It returns a deterministic AlphaDesk PASS/FAIL/unavailable result and signal score; it is assessment-only and is not broker authorization. \`prophet_place_options_order\` independently reassesses immediately before broker submission. Client-supplied or replayed assessments do not authorize execution, and AlphaDesk PASS alone never authorizes an order.
